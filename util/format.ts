...
const LIABILITY_INTEREST_MODE_LABELS: Record<string, string> = {
    MANUAL: "Manual",
    SIMPLE: "Simples",
    COMPOUND: "Composto"
};

const LIABILITY_TX_TYPE_LABELS: Record<string, string> = {
    OPENING: "Abertura",
    PAYMENT: "Pagamento",
    INTEREST: "Juros",
    FEE: "Taxa",
...
function labelFromMap(map: Record<string, string>, value?: string, fallback = ""): string {
 ...
 */
export function liabilityTypeText(value?: string): string {
 ...
 * @returns {string} the label, or the raw value when unknown
 */
export function liabilityStatusText(value?: string): string {
 ...
 * @returns {string} the label, or the raw value when unknown
 */
export function liabilityInterestModeText(value?: string): string {
 ...
 * @returns {string} the label, or the raw value when unknown
 */
export function liabilityTxTypeText(value?: string): string {
 ...
 */
export function liabilityInstallmentText(paid?: number | string, total?: number | string): string {
 ...
 */
export function liabilityYesNoText(value?: boolean): string {
 ...
 */
export function liabilityStatusState(status?: string): string {
    if (status === "OVERDUE") {
        return "Error";
    }
    if (status === "PAID") {
 ...
 */
export function liabilityProgressValue(value?: number | string): number {
 ...
 */
export function liabilityProgressText(value?: number | string): string {
    const parsed = Number(value) || 0;
    return `${parsed.toLocaleString("pt-BR", { maximumFractionDigits: 1 })}%`;
}
 ...
 * @returns {string} the UI state
 */
export function liabilityHealthState(value?: number | string): string {
 ...
 */
export function liabilityDatesText(start?: string, firstDue?: string): string {
    const parts: string[] = [];
    if (start) {
 ...
 * @returns {string} the summary
 */
export function liabilityInterestText(mode?: string, rate?: number | string): string {
    const modeText = liabilityInterestModeText(mode);
    const parsed = Number(rate);
    if (modeText && Number.isFinite(parsed)) {
        return `${modeText} • ${parsed.toLocaleString("pt-BR", { maximumFractionDigits: 4 })}%`;
    }
 ...
 */
export function liabilityOverdueText(label?: string, isOverdue?: boolean): string {
    if (!label) {
        return "";
    }
